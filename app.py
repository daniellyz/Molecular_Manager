...
# Route to get and post methods for measured_compounds

@app.route('/measured_compounds', methods=['POST', 'GET'])
    ...
    # If the compound does not exist in the compound database, consider update compound table if user has admin right:
        ...
        if cid not in ALLOWED_CID:
            if pw != ALLOWED_ADMIN_PW or user != ALLOWED_ADMIN_USER:
                return jsonify({"Achtung": "Compound does not exist with id: " + str(
                    cid) + ". You need admin right to add compound."}), 400
            else:
